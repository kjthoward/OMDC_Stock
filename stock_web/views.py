...
def _item_context(httprequest, item, undo):
    ...
    context = {"header":title,"headings":headings, "body":body, "toolbar":_toolbar(httprequest), "track_vol":False, "label": not item.printed}
...
def _vol_context(httprequest, item, undo):
    ...
    context = {"header":title,"headings":headings, "body":body, "toolbar":_toolbar(httprequest),"label":not item.printed}
    if ((item.finished==True) and (item.fin_text is not None)):
        context.update({"newinformation":item.fin_text})
